(function(root,factory){
  ...
  class MellowPreloadQueue{
    ...
    load(asset){const item=this.normalize(asset);if(!item?.url)return Promise.resolve(false);if(this.cache.has(item.url))return this.cache.get(item.url).promise;let element;const promise=new Promise(resolve=>{let settled=false;const done=ok=>{if(settled)return;settled=true;resolve(ok)};if(item.type==='audio'){element=new Audio();element.preload='auto';element.addEventListener('canplaythrough',()=>done(true),{once:true});element.addEventListener('error',()=>done(false),{once:true});element.src=item.url;element.load()}else{element=new Image();element.decoding='async';element.onload=async()=>{try{await element.decode?.()}catch(error){}done(true)};element.onerror=()=>done(false);element.src=item.url;if(element.complete&&element.naturalWidth)element.onload()}});this.cache.set(item.url,{promise,element,type:item.type});return promise}
    ...
    async preload(assets=[],options={}){const settings={showLoader:false,label:this.options.label,minimumMs:this.options.minimumMs,timeoutMs:this.options.timeoutMs,criticalTimeoutMs:8000,...options};const started=performance.now();const loader=settings.showLoader?this.mountLoader(settings.label):null;const normalized=assets.map(asset=>this.normalize(asset)).filter(Boolean);const work=Promise.allSettled(normalized.map(asset=>this.load(asset)));const critical=normalized.filter(asset=>asset.critical||asset.required);const criticalWork=critical.length?Promise.allSettled(critical.map(asset=>this.load(asset))):work;await Promise.race([criticalWork,new Promise(resolve=>setTimeout(resolve,critical.length?settings.criticalTimeoutMs:settings.timeoutMs))]);if(!critical.length)await Promise.race([work,new Promise(resolve=>setTimeout(resolve,settings.timeoutMs))]);const remaining=settings.minimumMs-(performance.now()-started);if(loader&&remaining>0)await new Promise(resolve=>setTimeout(resolve,remaining));if(loader){loader.classList.remove('is-visible');setTimeout(()=>loader.remove(),180)}return this}
